...
def _command_uses_single_worker_config(command):
    config = runpy.run_path(str(ROOT / 'gunicorn.conf.py'))
    explicit = [command[i + 1] for i, word in enumerate(command[:-1]) if word in ('-w', '--workers')]
    loads_config = any(word in ('-c', '--config') and i + 1 < len(command)
                       and command[i + 1] == 'gunicorn.conf.py'
                       for i, word in enumerate(command))
    return explicit == ['1'] or (not explicit and loads_config and config['workers'] == 1)
...
def test_procfile_never_starts_multiple_workers_with_process_local_financial_cache():
    ...
    assert _command_uses_single_worker_config(command), (
        'Production Gunicorn must either pin one worker or load the one-worker config')
