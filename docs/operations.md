...
4. Hosted PostgreSQL derives a stable session key from the existing private `DATABASE_URL`. An explicit `SECRET_KEY` (32+ characters) overrides it. Changing the DB URL or switching to an explicit key signs out existing sessions. Local SQLite without `DATABASE_URL` requires a key; the Windows launcher stores one in an ignored private file.
